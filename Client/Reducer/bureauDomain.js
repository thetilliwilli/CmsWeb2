...
export default function BureauReducer(state = initState.bureauDomain, action){
    switch(action.type)
    {
        ...
        //CATSUB
        case at.BUREAU_CATSUB_CHANGE: return CloneState(state, newState => {
            newState[`bureau${action.payload.mode}`].data.catsub = action.payload.catsub;
        });
        //CATSUB
        case at.BUREAU_CHANGE_COUNTRIES: return CloneState(state, newState => {
            newState[`bureau${action.payload.mode}`].data.countries = action.payload.countries;
        });
